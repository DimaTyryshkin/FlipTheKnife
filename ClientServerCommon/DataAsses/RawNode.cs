...
namespace OEPFramework
{
    public class RawNode
    {
        ...
        public Dictionary<string, T> GetDictionary<T>(string key, Func<string, RawNode, T> factory)
        {
            var node = GetNode(key);
            var dict = node.dictionary;

            var res = new Dictionary<string, T>();
            foreach (var pair in dict)
            {
                res.Add(pair.Key, factory(pair.Key, node.GetNode(pair.Key)));
            }
            return res;
        }

        public T[] GetArray<T>(string key, Func<RawNode, T> factory)
        {
            var node = GetNode(key);
            var list = node.array;
            ...
            for (int i = 0; i < arr.Length; i++)
                arr[i] = factory(node.GetNode(i));
            return arr;
        }

        public T[] GetArray<T>(string key, Func<RawNode, int, T> factory)
        {
            var node = GetNode(key);
            var list = node.array;
            ...
            var arr = new T[list.Count];
            for (int i = 0; i < arr.Length; i++)
                arr[i] = factory(node.GetNode(i), i);
            return arr;
        }
        ...
        public RawNode ExceptNodes(params string[] keys)
        {
            var dict = new Dictionary<string, object>(this.dictionary.Count);

            foreach (var item in this.dictionary)
            {
                if (Array.IndexOf(keys, item.Key) == -1)
                {
